...
class NaiveExecutionPlanner : ExecutionPlanner {
    ...
    override fun planExecution(query: Query, ctxt: PhysicalDatabaseProofSearchContext): PlanFunctor<Unit, Any> {
        return when(query) {
            is OrQuery -> UnionFunctor(
                query.goals
                    .map { planExecution(it, ctxt) }
                    .toTypedArray()
            )
            is AndQuery -> {
                when(query.goals.size) {
                    0 -> NoopFunctor() as PlanFunctor<Unit, Any>
                    1 -> planExecution(query.goals[0], ctxt)
                    else -> {
                        var pivot = FunctorPipe(
                            planExecution(query.goals[0], ctxt),
                            planExecution(query.goals[1], ctxt) as PlanFunctor<Any, Any>
                        )

                        for (i in 2..query.goals.lastIndex) {
                            pivot = FunctorPipe(
                                pivot,
                                planExecution(query.goals[i], ctxt) as PlanFunctor<Any, Any>
                            )
                        }
